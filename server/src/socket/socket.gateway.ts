...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
  ...
  private connectedClients = new Map<string, { userId?: string; apiKeyId?: string; username?: string; authType?: 'user' | 'api-key' }>();
        ...
        if (token) {
          ...
          try {
            ...
            if (payload.type === 'access' && payload.sub) {
              ...
              this.logger.log(`User authenticated via JWT: ${payload.username || payload.sub} (${socket.id})`);
              return next();
            }
          } catch (error) {
  ...
  handleConnection(client: AuthenticatedSocket) {
    ...
    if (client.authType === 'user') {
      this.logger.log(`Authenticated user connected: ${client.username || client.userId} (${client.id})`);
      client.emit('authenticated', { type: 'user', userId: client.userId, username: client.username });
    } else if (client.authType === 'api-key') {
      this.logger.log(`Authenticated API key connected: ${client.apiKeyId} (${client.id})`);
      client.emit('authenticated', { type: 'api-key', apiKeyId: client.apiKeyId, userId: client.userId });
    } else {
      this.logger.log(`Client connected: ${client.id} (unauthenticated)`);
      client.emit('warning', { message: 'Connection established without authentication. Some features may be limited.' });
    }
  }
  ...
  handleDisconnect(client: AuthenticatedSocket) {
    ...
    if (clientInfo?.authType === 'user') {
      this.logger.log(`Authenticated user disconnected: ${clientInfo.username || clientInfo.userId} (${client.id})`);
  ...
  handleMessage(@MessageBody() data: unknown, @ConnectedSocket() client: AuthenticatedSocket) {
    try {
      ...
      if ('message' in messageData && typeof (messageData as any).message === 'string') {
        ...
        const clientInfo = this.connectedClients.get(client.id);
        const username = parsed.username || clientInfo?.username || clientInfo?.apiKeyId || client.id;
        ...
        if (parsed.room) {
          ...
          this.logger.log(`Message in room "${parsed.room}" from ${username} (${client.id}): ${parsed.message}`);
        } else {
      ...
      if (parsed.room) {
        this.server.to(parsed.room).emit('message', messagePayload);
        this.logger.log(`Message in room "${parsed.room}" from ${from} (${client.id}): ${parsed.content}`);
      } else {
        this.server.emit('message', messagePayload);
  ...
  handleJoin(@MessageBody() data: { username?: string }, @ConnectedSocket() client: AuthenticatedSocket) {
    try {
      const clientInfo = this.connectedClients.get(client.id);
  ...
  handleLeave(@MessageBody() data: { username?: string }, @ConnectedSocket() client: AuthenticatedSocket) {
    try {
      const clientInfo = this.connectedClients.get(client.id);
      const username = data?.username || clientInfo?.username || clientInfo?.apiKeyId || client.id;
  ...
  handleJoinRoom(@MessageBody() data: { room: string }, @ConnectedSocket() client: AuthenticatedSocket) {
  ...
  handleLeaveRoom(@MessageBody() data: { room: string }, @ConnectedSocket() client: AuthenticatedSocket) {
    const roomSchema = z.object({ room: z.string() });
    try {
      const parsed = roomSchema.parse(data);
      const clientInfo = this.connectedClients.get(client.id);
