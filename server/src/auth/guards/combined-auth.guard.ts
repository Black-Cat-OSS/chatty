import { Injectable, ExecutionContext, BadRequestException, UnauthorizedException, Logger } from '@nestjs/common';
...
export class CombinedAuthGuard extends AuthGuard(['jwt', 'api-key-jwt']) {
  ...
  canActivate(context: ExecutionContext) {
    ...
    const hasAuthorization = request.headers.authorization && request.headers.authorization.startsWith('Bearer ');
    ...
    if (hasAuthorization && hasApiKey) {
      ...
      throw new BadRequestException('Cannot use both Authorization and X-API-Key headers simultaneously. Use only one authentication method.');
    }
  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    ...
    if (!hasAuthorization && !hasApiKey) {
      this.logger.warn(`Authentication required but not provided: ${request.method} ${request.url} from ${request.ip}`);
      throw new UnauthorizedException('Authentication required. Provide either Authorization Bearer token or X-API-Key header.');
    }

    // Есть токен, но он невалидный
    if (err) {
      this.logger.warn(`Authentication error: ${err.message || 'Unknown error'} - ${request.method} ${request.url} from ${request.ip}`);
      throw err;
    }
    
    if (info) {
      this.logger.warn(`Invalid token: ${info.message || 'Unknown error'} - ${request.method} ${request.url} from ${request.ip}`);
